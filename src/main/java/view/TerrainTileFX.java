...
public class TerrainTileFX extends StackPane {
    ...
    public TerrainTileFX(TerrainTile tile) {
        ...
        newOverlay = new Rectangle(70, 70, Color.rgb(0, 0, 255, 0.2));
    ...
    public void updateTileView() {
        ...
        if (this == GameController.getLastClicked()) {
            getChildren().addAll(newOverlay);
        }
        if (tile.isEmpty()) {
            return;
        }

        Color color = getTile().getOccupant().getColor();
        getChildren().addAll(new Rectangle(70, 70, color));
        icon.setImage(getTile().getOccupant().getImage());
