...
public class GameController {
    ...
    public static void setLastClicked(TerrainTileFX last) {
        ...
        if (state == GameState.ATTACKING) {
            ...
            if (move(last)) {
                ...
                alert.setTitle("Can't move selected unit");
                alert.setHeaderText("Unit can't be moved to selected tile");
    ...
    private static void attackTile(TerrainTile attacker, TerrainTile enemy) {
        if (enemy.isEmpty()
            ...
            state = GameState.NEUTRAL;
            Alert alert = new Alert(Alert.AlertType.WARNING);
            alert.setTitle("Can't attack selected unit");
            alert.setHeaderText("Selected unit can't be attacked");
            alert.setContentText("Try another action");
            alert.showAndWait();
            new AudioClip("File:./src/main/audio/alert.wav").play();
            return;
    ...
    private static boolean nearSettlement(TerrainTileFX tile) {
        mapSize = CivilizationGame.getMapSize();
    ...
    private static TerrainTile getRandomAdjacentTile(int row, int col) {
        ...
        while (!(r >= 0 && r < mapSize && c > 0 && c < mapSize)) {
