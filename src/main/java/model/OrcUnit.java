package model;

import javafx.scene.image.Image;

/**
 * Represents a Legion unit.
 *
 * @author Akarsh Hemrajani
 * @version 1.0
 */
class OrcUnit extends MeleeUnit {

    /**
     * Public constructor
     *
     * @param owner The owner of this unit.
     */
    public OrcUnit(Civilization owner) {
        super(owner);
        this.setDamage((int) (this.getDamage() * 1.5));
    }

    @Override
    public void battle(MapObject o) {
        o.damage(this.getDamage());
        if (!o.isDestroyed() && o instanceof MeleeUnit) {
            damage(((MilitaryUnit) o).getDamage());
        }
    }

    @Override
    public char symbol() {
        return 'O';
    }

    @Override
    public String toString() {
        return "Orcs. " + super.toString();
    }

    @Override
    public Image getImage() {
        return new Image("File:./src/main/java/view/Civ_Icon/orc_icon.PNG");
    }

}
