...
public class Map {
    ...
    private void populateMap(int rows, int columns) {
     ...
     */
    public static void addEnemies(Bandit bandit, int numEnemies, int mapSize) {
        for (int i = 0; i < numEnemies; i++) {
            int r = new Random().nextInt(mapSize);
            int c = new Random().nextInt(mapSize);
            putSettlement("Bandit Hideout", bandit, mapSize - 1, 0);
            while (!map[r][c].isEmpty()) {
                //try to get new coords
                r = new Random().nextInt(mapSize);
                c = new Random().nextInt(mapSize);
            }
